...
class ALCEmployee extends ___ALCObjectLinkable implements IALCEmployee
{	
	private $table_name  = '';
	...
	{
		$this->table_name = ALC_DATABASE_TABLE_PREFIX . 'alc_employees';
		...
		if (count($result) == 1) {
			...
			//$this->register_link_handler('Accounts', ALC_DATABASE_TABLE_PREFIX . 'alc_xlref_account_client', ALC_DATABASE_TABLE_PREFIX . 'alc_clients', ALC_DATABASE_TABLE_PREFIX . 'alc_accounts', 'ClientID', 'account_id', 'ALCAccounts', 'ALCAccount');
	...
	final public function data_time_created() 
	{
		return ALC::library('ALCDates')->Reverse($this->properties['data_time_created']);
	}
